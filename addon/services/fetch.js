...
import Error from '@rancher/ember-shared/models/error';
...
export default Service.extend({
  ...
  request(url, opt) {
    ...
    opt.url = this._normalizeUrl(url, opt);
  ...
  _normalizeUrl(url, opt) {
    opt = opt || {};
    const includingAbsolute = opt.includingAbsolute === true;
    const addBase  = opt.addBase !== false;
  ...
  fetch(url, opt) {
    ...
    opt = opt || {};
    opt.url = this._normalizeUrl(opt.url, opt);
